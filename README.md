...
3. 이번 회차를 **기본 회차로** 버튼으로 지정 → 학부모에겐 **고정 주소**(`https://<배포주소>/`, `?s=` 없이)를 가정통신문·QR로 배포 (학부모 개인정보 수집 동의는 앱 로그인 화면에서 받음)
   - 학부모 주소는 매 회차 **그대로**, 관리자가 **기본 회차만 다시 지정**하면 됩니다. (기본 회차 미지정 시 학부모 화면은 "준비 중"으로 안전하게 표시)
   - 여러 회차를 동시에 운영할 때만 "🔗 회차 링크"의 **링크 복사**(`?s=학년도-회차`)를 사용하세요.
